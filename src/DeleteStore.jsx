...
const DeleteStore = (id) => {
    ...
    let deletedata = async (id) => {
        if (window.confirm("Are you Sure")) {
            let result = await fetch(`http://localhost:4500/deletedata/${params.id}`, {
                method: 'delete',
                body:JSON.stringify(),
                headers: {
    ...
    return (
        <div style={{ marginLeft: '20px' }}>
            <h3 style={{ margin: '20px' }}>STORE/<span style={{ fontSize: '17px', color: 'grey' }}>DELETE STORE</span></h3>
            <hr />
            <h4 style={{ textAlign: 'left', marginTop: '5px', color: "grey" }}>Store Information</h4>
                    <div class='row' >
                        <div class='col-sm-6'>
                           ...
                           <Link ><Button variant='contained' onClick={()=>deletedata(params.id)} style={{ marginTop: '20px', background: '#00ACFF', height: '42px' }}>Delete</Button></Link> 
                        </div>
                        <div class='col-sm-6'>
